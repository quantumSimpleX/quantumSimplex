...
function Nav({ page, onNav, menuOpen, setMenuOpen }) {
  const { nav } = window.QSData;
  const heroDarkPages = ['home', 'about', 'services', 'insights', 'engage'];
  const [scrolled, setScrolled] = React.useState(false);
  const [isDark, setIsDark] = React.useState(() => {
    try { return localStorage.getItem('qs-theme') === 'dark'; } catch(e) { return false; }
  });

  // Apply theme to document root
  React.useEffect(() => {
    document.documentElement.setAttribute('data-theme', isDark ? 'dark' : 'light');
    try { localStorage.setItem('qs-theme', isDark ? 'dark' : 'light'); } catch(e) {}
  }, [isDark]);
  ...
  // In dark mode the whole site is dark — header is always dark-styled
  const darkHeader = isDark || (heroDarkPages.includes(page) && !scrolled);

  return (
    <>
      <header className={'qs-header' + (darkHeader ? ' on-dark' : '')}>
        <a className="qs-header-brand" href="#" onClick={e => { e.preventDefault(); onNav('home'); }}>
          <QSMark onDark={darkHeader} />
          <span className="qs-header-wordmark">
            <span className="w-row" style={{ transform: 'translateY(7px)' }}><span className="w-main">UANTUM</span></span>
            <span className="w-row" style={{ paddingLeft: '16px' }}><span className="w-main">SIMPLE</span><span className="w-x" style={{ fontSize: '28px', letterSpacing: '0.8px' }}>X</span></span>
          </span>
        </a>
        ...
        <div className="qs-header-controls">
          <button className="qs-theme-toggle" onClick={() => setIsDark(d => !d)}
                  aria-label={isDark ? 'Switch to light mode' : 'Switch to dark mode'}>
            {isDark ? <SunIcon /> : <MoonIcon />}
          </button>
          <button className="qs-menu-toggle" aria-label="Menu"
